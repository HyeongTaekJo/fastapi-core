...
from post.schemas.request import PaginatePostSchema, CreatePostSchema, UpdatePostSchema
...
class PostRepository:
    ...
    async def get_posts(self) -> List[PostModel]:
        result = await self.db.scalars(select(PostModel))
        return list(result)
    ...
    async def delete_post(self, id: int) -> None:
        ...
        await self.db.execute(delete(PostModel).where(PostModel.id == id))
    ...
    async def generate_posts(self):
        ...
        await self.db.execute(insert(PostModel).values(posts))
